...
class ShowSearcher:
    """
    Handles user queries and returns show search results
    """
    ...
    async def processResult(self, results: list[ShowSearch], absLink: str) -> None:
        async with httpx.AsyncClient() as client:
            ...
            results.append(showResponse.dict())

    async def processLink(self, showResults: str) -> list[Dict[str, Any]]:
        results = []
        soup = BeautifulSoup(showResults, "html.parser")
        showDetails = soup.find_all("div", {"class": "film-detail"})
        showIds = [show.find("a").get("href") for show in showDetails]
        for showId in showIds:
            if "tv/watch" in showId:
                await self.processResult(results, self.url + showId)
        return results

    async def main(self) -> dict[str, list[Dict[str, Any]]]:
        async with httpx.AsyncClient() as client:
            showResults = await client.get(
                f"{self.url}/search/{self.query.replace(' ', '-')}", timeout=10
            )
            results = await self.processLink(showResults.text)
